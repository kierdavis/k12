...
func (em *Emulator) Step() {
	...
	if inst & 0x0800 != 0 {
		...
		switch (inst >> 12) & 0xF {
		case 0x0: handler = (*Emulator).doInc
		case 0x1: handler = (*Emulator).doIn
		case 0x2: handler = (*Emulator).doLdi
		case 0x3: handler = (*Emulator).doLdd
		case 0x4: handler = (*Emulator).doDec
		case 0x5: handler = (*Emulator).doOut
		case 0x6: handler = (*Emulator).doSti
		case 0x7: handler = (*Emulator).doStd
		case 0x8, 0x9, 0xA, 0xB:
			handler = (*Emulator).doSkip
		case 0xC: handler = (*Emulator).doRjmp
		case 0xD: handler = (*Emulator).doRcall
		case 0xE: handler = (*Emulator).doLjmp
		case 0xF: handler = (*Emulator).doHalt
		}
	}
...
func (em *Emulator) doSkip(inst uint16) {
	...
	switch (inst >> 8) & 0x7 {
	case 0x0: flag = zero
	case 0x1: flag = negative
	case 0x2, 0x4: flag = borrow
	case 0x3: flag = overflow
	case 0x5: flag = borrow || zero
	case 0x6: flag = slt
	case 0x7: flag = slt || zero
	}
